...
const nc = PythonCall.pyimport("numcodecs")
...
@testset "Fletcher32Filter" begin
    ...
    for Typ in (UInt8, Int32, Float32, Float64)
        ...
        # Test using numcodecs' Fletcher32 filter
        filter = Fletcher32Filter()
        dict = Zarr.JSON.lower(filter)
        numcodecs_filter = if isempty(setdiff(keys(dict), ["id"]))
            nc.Fletcher32()
        else
            nc.Fletcher32.from_config(filter)
        end
        
        encoded = zencode(arr, filter)
        nc_encoded = pyconvert(Vector{eltype(encoded)}, numcodecs_filter.encode(arr))
        @test encoded == nc_encoded
        
        decoded = zdecode(encoded, filter)
        nc_decoded = pyconvert(Vector{eltype(decoded)}, numcodecs_filter.decode(encoded))
        @test reinterpret(Typ, decoded) == reinterpret(Typ, nc_decoded)
    end
    ...
    for codec in codecs
        for array in arrays
            try
                ...
                if eltype(array) <: Union{Zarr.MaxLengthString, DateTime64, Nanosecond, Minute}
                    continue
                end

                # Test using numcodecs' Shuffle filter
                ...
                numcodecs_filter = if isempty(setdiff(keys(dict), ["id"]))
                    ...
                    pop!(dict, "id")
                    nc.Shuffle.from_config(pydict(dict))
                end
                nc_encoded = pyconvert(Vector{eltype(encoded)}, numcodecs_filter.encode(pyimport("numpy").array(array)))
                @test encoded == nc_encoded
                
                decoded = zdecode(encoded, filter)
                nc_decoded = pyconvert(Vector{eltype(decoded)}, numcodecs_filter.decode(pyimport("numpy").array(encoded)))
                @test decoded == nc_decoded
            catch error
                @warn "Error in ShuffleFilter test: $error\nat array = ($(typeof(array)), $(length(array))), codec = $codec"
                rethrow(error)
            end
        end
    end
